...
module BugLocation = struct
  ...
  let pp fmt (l, score_neg, score_pos, score_time) = F.fprintf fmt "%s:%d\t%f %f %f" l.Cil.file l.Cil.line score_neg score_pos score_time
end
  ...
  List.fold_left
    (fun locs elem ->
      Coverage.StrMap.fold
        (fun file lines locs ->
          let new_locs =
            List.map (fun line -> ({ Cil.file; line; byte = 0 }, 0.0, 0.0, 0.0)) lines
          in
          locs @ new_locs)
...
let prophet_localizer work_dir bug_desc =
  ...
  let locations =  List.fold_left
    (fun locs elem ->
      ...
      Coverage.StrMap.fold
        (fun file lines locs ->
          let new_locs =
          if Str.string_match regexp_pos elem.LineCoverage.test 0
          then List.map (fun line -> ({ Cil.file; line; byte = 0 }, 0.0, 1.0, 0.0)) lines
          else List.map (fun line -> ({ Cil.file; line; byte = 0 }, 1.0, 0.0, 0.0)) lines
          in
          locs @ new_locs)
        elem.LineCoverage.coverage locs)
    [] coverage
  ...
  let new_locations = List.map (fun (l, (s1, s2, s3)) -> (l, s1, s2, s3)) (List.of_seq (Hashtbl.to_seq table)) in
  List.stable_sort (fun (_, s11, s12, s13) (_, s21, s22, s23) ->
    if (s21-.s11)<>0. then int_of_float (s21-.s11)
    else if (s12-.s22)<>0. then int_of_float (s12-.s22)
    else int_of_float (s23-.s13) ) new_locations
